...
Migrator.prototype.performMigration = function(direction, migrationName, cb) {
  ...
  this.setup(function(err) {
    ...
    self.migration.all(function(err, appliedMigrations) {
      if(err) return cb(err);
      ...
      if(direction === 'up') { // up -> reject the applied migrations
        ...
        if (_.isEmpty(migrationModules)) {
          log(direction, 'No migrations to revert');
        } else if(_.isEmpty(migrationName)) {
          migrationModules = [ _.head(migrationModules) ]
        }
      }
